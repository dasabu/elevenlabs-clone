...
from gen_wav import initialize_model, gen_wav, SAMPLE_RATE
...
S3_PREFIX = os.getenv("S3_PREFIX", "make-an-audio-output")
...
app = FastAPI(title="Make-An-Audio API", lifespan=lifespan)
...
async def generate(request: GenerateRequest, background_tasks: BackgroundTasks):
    if not sampler and vocoder:
        raise HTTPException(status_code=500, detail="Models not loaded")

    try:
        wav_list = gen_wav(
            sampler=sampler, vocoder=vocoder, prompt=request.prompt,
            ddim_steps=100, scale=3.0,duration=10, n_samples=1
        )
        ...
        raise HTTPException(
            status_code=500, detail="Error in generating audio")

@app.get("/health", dependencies=[Depends(verify_api_key)])
async def health_check():
    if sampler and vocoder:
