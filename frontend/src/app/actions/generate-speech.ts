...
export async function generateSpeechToSpeech(originalVoiceS3Key: string, voice: string) {
  const session = await auth()
  if (!session?.user.id) {
    throw new Error("User not authenticated")
  }
...
// Polling (every 500ms)
export async function generateStatus(audioId: string): Promise<{success: boolean, audioUrl: string | null}> {
  const session = await auth()
  ...
  if (audioClip.s3Key) {
    ...
    return { success: true, audioUrl: await getPresignedUrl({ key: audioClip.s3Key }) }
  }
