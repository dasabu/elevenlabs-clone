...
import { type SignUpFormValue, signUpSchema } from "~/schemas/auth"
...
export default function SignUpPage() {
  ...
  const { register, handleSubmit, formState: {errors}, watch } = useForm<SignUpFormValue>({
    resolver: zodResolver(signUpSchema),
    defaultValues: {
      email: "",
      password: ""
    },
    // validation runs whenever user types instead of just submit
    mode: "onChange"
  })
  ...
  const onSubmit = async (data: SignUpFormValue) => {
    ...
    // trigger authorize function in server/auth/config.ts
        ...
        <div className="hidden h-full rounded-3xl bg-gradient-to-b from-indigo-100 via-purple-100 to-[#5960d7] lg:block">
          <div className="flex h-full flex-col p-12">
            ...
            <div className="h-fit w-full max-w-lg">
              <div className="mb-3 flex w-fit rounded-2xl bg-indigo-300 bg-opacity-40 px-3 py-1">
                <span className="text-xs font-medium uppercase tracking-wider text-white">
                  Latest updates
                </span>
              </div>
              <h3 className="text-lg text-white xl:text-xl 2xl:text-2xl 2xl:leading-10">
                Use the text-to-speech editor to create voice overs in multiple
                voices using AI.
              </h3>
            </div>
          </div>
